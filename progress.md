...
### June 2026: Advanced Customization & Flyer Overhaul (Phase 2 Completed)
* **Logo Center Branding Fix**: Reprogrammed QR Art generation preview so it properly honors the `logoEnabled` checkbox toggle instead of ignoring it.
* **QR Background & 3D Depth Expansion**: Fixed non-functional background textures by adjusting alpha opacity levels. Expanded selects with new premium patterns (`dots`, `circuit` textures) and advanced 3D effects (`layered`, `chiseled`, `floating` offsets). Added pointed `star_tips` and `shield` corner frame shapes. Added `duotone` and `inverse` QR color options.
* **CTA Banner Size Increase**: Doubled the font size of CTA banner texts (e.g. `SCAN ME TO CONNECT`) on the generated QR asset from `14px` to `20px` to enhance readability.
* **Business Card Interactive Fields**: Moved the background textures draw layer after theme colors to prevent overwriting. Allowed users to dynamically add/remove up to 3 custom text fields to print on the card.
* **Stand Flyer Split Column Overhaul**: Hidden WiFi details fields when WiFi is not added. Split flyer details section into a two-column editor configuration (**Business QR Flyer** and **WiFi QR Flyer**) when WiFi is added. Exposed business flyer editing fields for titles, subtitles, logo header, and specific section displays (Menu, WhatsApp, Socials, Contact). Added five premium flyer themes (`forest_dark`, `ocean_dark`, `warm_sunset`, `rose_gold`, `slate_blue`) and two flyer textures (`hexagon`, `noise`).